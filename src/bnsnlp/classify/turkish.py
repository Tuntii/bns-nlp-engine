...
class TurkishClassifier(BaseClassifier):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.intent_model_name = config.get('intent_model', 'savasy/bert-base-turkish-sentiment-cased')
    ...
    async def classify(self, text: Union[str, List[str]]) -> Union[ClassifyResult, List[ClassifyResult]]:
    ...
    async def _classify_single(self, text: str) -> ClassifyResult:
        ...
        try:
            ...
            intent_result = await loop.run_in_executor(
                self._executor,
                lambda: self._intent_pipeline(text)[0]
            )
            
            # Get entities
            entity_results = await loop.run_in_executor(
                self._executor,
                lambda: self._entity_pipeline(text)
            )
    ...
    async def _classify_batch(self, texts: List[str]) -> List[ClassifyResult]:
        ...
        try:
            ...
            intent_results = await loop.run_in_executor(
                self._executor,
                lambda: self._intent_pipeline(texts)
            )
            
            # Get entities for all texts
            entity_results = await loop.run_in_executor(
                self._executor,
                lambda: [self._entity_pipeline(text) for text in texts]
            )
